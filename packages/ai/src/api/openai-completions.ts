...
function createClient(
	...
	// I used to copy this portion to /Users/alfredo/.nvm/versions/node/v22.22.3/lib/node_modules/@earendil-works/pi-coding-agent/node_modules/@earendil-works/pi-ai/dist/api/openai-completions.js
	// ==========================================================================
	...
	if (boolAddFetch) {
		const fetchToAdd: typeof fetch = async (url, init) => {
			...
			if (!boolLogOnlyWhenError || (boolLogOnlyWhenError && !response.ok)) {
				...
				const logFile = path.join(logDir, `openai-completions-${timestamp}.log`);
				let didWrite = false;
